...
export async function addDocumentToVault(params: {
  userId: string;
  fileUri: string;
  filename: string;
  mimeType: string;
  ...
  if (sizeBytes > MAX_DOC_SIZE_BYTES) {
    throw new Error('Documento muito grande (limite 25MB).');
  }
...
export async function getDocumentFromVault(
  ...
  if (item.encrypted_metadata) {
    try {
      ...
      // se metadata corrompida, segue com default
    }
  }
  ...
  const [ciphertext, iv] = payloadText.split(':');
  if (!ciphertext || !iv) {
    throw new Error('Documento corrompido.');
  }

  const base64 = await decryptString(ciphertext, iv, key);

  const tempPath = `${FileSystem.cacheDirectory}vault_temp_${itemId}_${filename}`;
...
export async function deleteDocumentFromVault(userId: string, itemId: string): Promise<void> {
  const { data: item } = await supabase
    .from('vault_items')
    .select('storage_path')
    .eq('user_id', userId)
